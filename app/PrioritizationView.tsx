...
type Item = { id: string; title: string; kind: string; state: string; status: string; points: number | null; age: number; currentIterationId: string | null };
...
type ScoreInput = { business: number; time: number; risk: number; jobSize: number | null; confidence: "High" | "Medium" | "Low"; evidence: string; rationale: string; decision: string; plannedSprint: string; mandatory: boolean };
...
const displayEventValue = (value: unknown, fallback = "Unassigned") => typeof value === "string" && value.trim() ? value : fallback;
const planningEventSummary = (event: PlanningEvent) => {
  if (event.eventType === "sprint") return `Moved from ${displayEventValue(event.from)} to ${displayEventValue(event.to)} · Source iteration: ${displayEventValue(event.sourceIterationId)} · Rationale: ${displayEventValue(event.rationale, "Not provided")}`;
  if (event.eventType === "estimate") return `Local Job Size: ${typeof event.points === "number" ? event.points : "Unestimated"} · Rationale: ${displayEventValue(event.rationale, "Not provided")}`;
  if (event.eventType === "reconciliation") return `Planned iteration: ${displayEventValue(event.plannedIterationId)} · Source iteration: ${displayEventValue(event.sourceIterationId)} · Result: ${displayEventValue(event.result, "Unavailable")}`;
  const previous = event.previous && typeof event.previous === "object" ? event.previous as { target?: unknown; max?: unknown } : {};
  const next = event.next && typeof event.next === "object" ? event.next as { target?: unknown; max?: unknown } : {};
  return `Capacity target: ${previous.target ?? "—"} → ${next.target ?? "—"} · Hard maximum: ${previous.max ?? "—"} → ${next.max ?? "—"}`;
};
...
const initial = (item: Item): ScoreInput => ({
  business: 0, time: 0, risk: 0, jobSize: item.points, confidence: "Low", evidence: "",
...
export default function PrioritizationView({ projectName, items, iterations }: { projectName: string; items: Item[]; iterations: Iteration[] }) {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        setInputs((current) => Object.fromEntries(Object.entries({ ...current, ...data.inputs }).map(([id, value]) => [id, { ...(value as ScoreInput), jobSize: items.find((candidate) => candidate.id === id)?.points ?? (ALLOWED_JOB_SIZES.includes((value as ScoreInput).jobSize as number) ? (value as ScoreInput).jobSize : null), plannedSprint: normalizeSavedSprint((value as ScoreInput).plannedSprint, planningHorizon) }])));
  ...
  const reconciliationFor = (item: Item) => reconcilePlan({ plannedIterationId: inputs[item.id].plannedSprint, sourceIterationId: sourceAssignments[item.id], sourceItemExists: item.state !== "Removed", iterations });
  ...
  const recordLocalEstimate = (item: Item) => {
    if ((sourcePointsFor(item) ?? 0) > 0) { setNotice(`${item.id} has a read-only source estimate; local estimate entry is disabled.`); return; }
    if (!ALLOWED_JOB_SIZES.includes(inputs[item.id].jobSize as number)) { setNotice(`${item.id}: choose Job Size 1, 2, 3, 5, 8, or 13 first.`); return; }
  ...
  const record = (item: Item) => {
    ...
    const nextHistory = appendDecisionSnapshot(history, {
      ...
      inputs: { business: x.business, time: x.time, risk: x.risk, jobSize: x.jobSize as number, confidence: x.confidence },
    });
  ...
  const refreshSourceSnapshot = () => {
    const permission = validateReadOnlyConnectorOperation("read", "human-refresh");
    if (!permission.allowed) { setNotice(permission.reason); return; }
    const missing=items.find((item)=>item.points===null);
    if(missing){setSourcePointRefresh((current)=>({...current,[missing.id]:5}));change(missing.id,"jobSize",5);}
  ...
  return <section className="priority-workspace" aria-label={`${projectName} prioritization`}>
    ...
    <section className="decision-history panel">
      ...
      {history.length ? <>
        ...
        <div className="history-list">{history.slice(0,8).map((event)=><article key={event.id}><time>{event.at} UTC</time><strong>{event.itemId} · priority v{event.decisionVersion}</strong><span>{event.method}</span><span>{event.decision}</span><span>Rank {event.currentRank} → {event.recommendedRank ?? "—"}</span><small>{event.actor} • {event.participants} • Evidence: {event.evidence} • Inputs: {event.inputs.business}/{event.inputs.time}/{event.inputs.risk}/{event.inputs.jobSize}, {event.inputs.confidence} confidence • {event.rationale || "No rationale entered"}</small></article>)}</div>
      ...
      {planningHistory.length ? <div className="planning-history">{planningHistory.slice(0,10).map((event)=><article key={event.id}><time>{event.at} UTC</time><strong>{event.eventType} · {event.subjectId} · v{event.version}</strong><span>{event.actor} • {event.participants}</span><small className="planning-event-summary">{planningEventSummary(event)}</small></article>)}</div> : <div className="empty-state compact"><strong>No planning changes recorded</strong><span>Changing a sprint, capacity limit, or recording a local estimate creates its own event.</span></div>}
    </section>
