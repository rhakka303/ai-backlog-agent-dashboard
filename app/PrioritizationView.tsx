...
type History = {
  id: string; itemId: string; at: string; method: Method; formulaVersion: string; score: number | null;
...
export default function PrioritizationView({ projectName, items }: { projectName: string; items: Item[] }) {
  ...
  const candidateInputs = candidates.map((item) => inputs[item.id]);
  const scoreFor = (item: Item) => calculateScore(method, inputs[item.id], weights, candidateInputs);
  const eligibilityFor = (item: Item) => classifyEligibility(inputs[item.id]);
  const currentRank = (item: Item) => items.findIndex((candidate) => candidate.id === item.id) + 1;
  ...
  const record = (item: Item) => {
    ...
    const event: History = {
      ...
      formulaVersion: FORMULA_VERSIONS[method], score: scoreFor(item), currentRank: currentRank(item),
