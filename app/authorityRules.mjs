...
export function revokeDelegation(delegation, revokedAt) {
  if (!delegation || delegation.revokedAt) throw new Error("Only an active delegation can be revoked.");
  const at = Date.parse(revokedAt); if (!Number.isFinite(at)) throw new Error("Revocation timestamp must be valid UTC.");
  return Object.freeze({ ...delegation, revokedAt: new Date(at).toISOString() });
}

export function delegationStatus(delegation, now) {
  const at = Date.parse(now); if (!delegation || !Number.isFinite(at)) return "invalid";
  if (delegation.revokedAt) return "revoked";
  if (at < Date.parse(delegation.startsAt)) return "scheduled";
  if (at >= Date.parse(delegation.expiresAt)) return "expired";
  return "active";
}

export function canFinalizeDecision({ authenticated, role, delegation, now }) {
  if (!authenticated) return false;
  if (role === "product-owner") return true;
  return role === "authorized-delegate" && delegationStatus(delegation, now) === "active";
}

export function authorityLabel(role, authenticated = true) {
  if (!authenticated) return "Unauthenticated user";
  if (role === "product-owner") return "Product Owner";
  if (role === "authorized-delegate") return "Authorized Delegate";
  return "Participant";
}
